...
- 敌军、活动炮弹、持续效果、掉落物、辅助系统与 UI 的结构完整性回归；
- 1440×810 默认窗口与 OpenGL Compatibility 启动回归；
- 2026-07-27 用户确认本轮不执行性能跑分、Profiler 调优或 60 FPS 指标门禁，集中完成玩法功能与独立构建验收；
- 对象池、分帧查询和更新频率调整统一延期，除非未来以独立性能迭代重新确认。

同时执行 Windows Debug/Release 边界、独立启动、固定种子、输入焦点、窗口暂停、完成/失败和重复开局回归，并交付启动说明、版本信息和已知限制。

---

## 14.1 BC-05A～BC-05B：发布候选可调试性与构筑可读性

- BC-05A 提供仅 Debug 可见的节奏调参、正式 Profile 保存和恢复入口；
- BC-05B 移除敌军程序底板，补强玩家受击，并把四部门协议和辅助机三阶等级投影到坦克外观；
- 两项均不得改变三核心、五波、奖励、敌军行为、Boss 和单区结算规则；
- 完成全部开发后统一执行构建、相关 headless、主场景启动和策划可见验收。
...
性能压力流程已于 2026-07-27 按用户确认从当前 BC-05 门禁移除；未来只有单独确认性能迭代后再执行。
