...
`game1` 最初是用于确认本机 Godot 4.7 .NET / C# 工具链的单场景移动与碰撞演示，之后已在同一项目中演进为当前的单房间三波中继站防守原型；当前实现的权威基线见第 21 节。早期 `Main.cs` 和 `GameRules.cs` 的演示结构不作为正式架构约束，后续也不建立第二套引擎工程。
...
| 逻辑画布 | 480×270，16:9；默认窗口 1440×810，以整数 3 倍显示。 |
...
480×270 用于形成现代高精度像素画面，同时在 1440×810 下得到无插值的 3 倍显示。所有像素纹理关闭过滤和 mipmap；摄像机最终渲染位置吸附到整数像素，但游戏物理位置不取整。
...
目标硬件以当前 OpenGL 3.3 兼容环境为下限参考，默认输出 1440×810，稳定 60 FPS。
...
### 20.1 Alpha 城区战场尺度基线（2026-07-19）

MVP 技术完成定义已满足。Alpha 城区内容在扩充前统一采用 24 像素格、约一格大小的普通坦克、底部中继站防区和上半区敌军出生边。房间坐标继续属于场景与 `RoomDefinition`；运行时代码不得假定固定坐标。玩家表现动画从场景读取基准缩放，只叠加运动脉冲，避免表现代码覆盖关卡尺度配置。

## 21. 当前实现基线与受限暂缓项

### 21.1 已符合

截至 2026-07-17，当前实现已经符合以下技术基线：

- 引擎与语言为 Godot 4.7 .NET / C#；
- 游戏程序集目标框架为 `net8.0`，独立测试项目目标框架为 `net10.0`；
- 逻辑画布为 480×270；
- 渲染器使用 OpenGL Compatibility；
- 单局耐久、装甲、重启次数等持久战斗状态由普通 C# `RunState` 持有；
- 炮弹采用物理帧线段扫掠查询并支持钢墙反弹；
- 伤害统一通过 `IDamageable` / `DamageContext` / `DamageResult` 契约结算；
- 表现节点通过事实信号订阅规则结果，表现与战斗规则保持分离。

### 21.2 当前原型简化

当前单房间守点与表现验证原型仍有 6 项受限简化。完整且唯一的清单见[0001 决策记录“已发生的简化”](../../decisions/0001-prototype-architecture-deferrals.md#已发生的简化)。这些简化不改变本文的目标架构，也不授权在后续迭代继续沿用。

### 21.3 后续强制门槛

每项暂缓内容的偿还时点和禁止条件，逐条以[0001 决策记录“强制偿还门槛”](../../decisions/0001-prototype-architecture-deferrals.md#强制偿还门槛)为准；本文不复制另一版本，避免门槛产生分叉。任何门槛变更均属于实质偏离。

### 21.4 治理入口

后续开发、审查、验证和偏离处理遵守[《废土中继》多智能体协同与项目治理设计](./2026-07-17-agent-collaboration-governance-design.md)及仓库根目录的[`AGENTS.md`](../../../AGENTS.md)。
