...
| 测试运行时 | 独立测试项目目标 `net10.0`，引用 `net8.0` 游戏程序集；Godot headless 集成测试仍在引擎实际 .NET 运行环境中执行。 |
...
每个可运行迭代除自动化测试外，必须提供一条无需阅读代码的策划验收路径：

- 画面明确展示本迭代的关键状态、数值与阶段；
- 关键对象通过轮廓、颜色、预警或文本提示表现职责差异；
- 验收说明统一采用“操作 → 预期画面/数值/状态变化”的格式；
- 原型期允许提供独立、明确标注的调试入口，以覆盖正常战斗暂时难以触发的边界；
- 开发者必须先按该路径自验，再交由策划验收；自动化通过不能替代可见验收。
