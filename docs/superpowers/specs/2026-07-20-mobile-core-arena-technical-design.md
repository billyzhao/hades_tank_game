...
- 出生候选还必须通过敌军实际碰撞体的地形/边界重叠查询，并且到玩家存在非空导航路径；任一条件失败时跳过该候选及本次生成，绝不生成可能卡死的敌军；
...
导航继续使用 `NavigationGrid` / `IEnemyPathProvider`。地形破坏后局部刷新；无路径时敌人停止并按退避间隔重试，不能直线穿墙。入口安全判断、实际碰撞净空与寻路是独立条件：入口可生成不代表路径必然有效，导演必须过滤或回退到另一入口；若没有同时通过三项检查的入口，则本次不生成敌军。
...
2026-07-28 增加 Debug 策划调参台约束：

- 菜单分为“快捷验收”和“节奏调参”，容器负责排版并保留键盘/手柄焦点；
- 可调项仅限全局刷怪速度、场上敌军上限修正、敌军移动/攻击频率/装甲倍率、玩家移动/射击频率倍率；
- `AcceptanceMenu` 只维护工作副本并发出类型明确的请求；`AppRoot` 负责把请求路由到 `WaveDirector`、敌军实例和玩家正式组件；
- 正式来源为单独的 `BlockadeCityBalanceProfile.tres`。Release 读取该资源，但不包含调参或保存入口；
- “保存为正式配置”只能在 Godot 编辑器启动的 Debug 运行中使用，保存前必须二次确认，保存后必须重新加载校验；
- 运行时调参不直接改写 `WaveDefinition`、`EnemyDefinition`、玩家场景或其他只读资源；
- 调参初始值从正式 Profile 读取；恢复已保存配置、恢复设计基准和复制参数快照必须可见；
- 新刷怪速度立即影响下一次刷新，数量上限立即影响后续补兵；敌军倍率只作用于新出生单位，避免重写场上实体的当前生命和状态机；玩家倍率立即生效；
- Profile 参数必须经过有限值与范围校验，保存失败必须明确显示，禁止伪成功。
...
原计划在 BC-05 执行的完整 30 敌人、160 炮弹和持续效果性能压力测试，已于 2026-07-27 按用户确认延期；当前发布候选只保留启动冒烟、结构数量与功能完整性检查，性能指标另立迭代。
...
2026-07-28 用户确认补强敌军透明表现、玩家受击、协议装配和辅助机进阶外观。该调整是既有构筑事实的表现投影，不增加新的玩法状态。

### 21.1 数据与职责

- `BuildSnapshot` 公开只读 `OwnedProtocols`，使表现层能按部门和等级建立视图；可写真值仍只在 `RunState`；
- `TankBuildVisualCatalog.tres` 以四个 `TankVisualSlot` 映射四部门模块，并以稳定辅助 Id 映射 Mk.I～Mk.III 贴图；
- `TankBuildVisualController` 订阅 `BuildController.SnapshotChanged`，只根据协议的 `Department` 和 `Rank` 生成模块；禁止在 `PlayerTank` 或表现控制器内按具体协议 Id 分支；
- `AuxiliaryHost` 继续执行辅助行为，并对现有槽位增量同步外观；普通属性快照变化不得销毁重建未变化的辅助节点；
- 重新创建玩家实例时，从完整快照重建外观，不新增单独存档字段。

### 21.2 受击边界

- 有效装甲下降触发受击序列、车体/炮塔闪色、构筑模块与辅助机闪色、HUD 装甲反馈；
- 机械位移只能作用于视觉子节点，禁止写入玩家物理坐标、速度、碰撞层或瞄准方向；
- 治疗、装甲上升和无伤事件不得误触发红色受击表现。

### 21.3 敌军与验收边界

- `enemy_tank.tscn` 不再保留程序 `Polygon2D` 背景板，敌军身份只由透明职责贴图和既有精英/受击调制表达；
- Debug 菜单通过信号请求四部门协议和四种辅助，由 `AppRoot` 调用 `BuildController` 正式入口；Release 边界保持不变；
- Batch 10 只记录视觉素材，图片不得反向决定伤害、射速、槽位、碰撞或 AI。
